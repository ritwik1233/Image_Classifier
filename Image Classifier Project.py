...
def network_model_setup():
    ...
    classifier = nn.Sequential(OrderedDict([
                          ...
                          ('inputLayer', nn.Linear(25088, 500)),
                          ('relu1', nn.ReLU()),
                          ('HiddenLayer', nn.Linear(500, 200)),
                          ('relu2', nn.ReLU()),
...
#create new model,criterion,optmizer and initialise the data
model,criterion,optimizer,device = network_model_setup()


# In[7]:


#determine whether the type of processor (Cuda or Cpu)
epoch=5
print_every=5
steps=0
loss_show=[]
...
for e in range(epoch):
    ...
    for ii,(inputs,labels) in enumerate(trainingloader):
        steps+=1
        inputs,labels=inputs.to(device.type),labels.to(device.type)
        optimizer.zero_grad()
        ...
        if steps%print_every==0:
            ...
            for ii,(vinputs,vlabels) in enumerate(validationloader):
                optimizer.zero_grad()
                vinputs,vlabels=vinputs.to(device.type+':0'),vlabels.to(device.type+':0')
                model.to(device.type+':0')
                with torch.no_grad():
            ...
            print("Epoch: {}/{}... ".format(e+1, epoch),
                ...
                "Validation Lost: {:.4f}".format(running_loss/print_every),
                "Accuracy  Loss: {:.4f}".format(running_loss/print_every))
            running_loss=0
...
def check_model_accuracy(model,testloader):
    correct=0
    total=0
    model.to('cuda:0')
    with torch.no_grad():
    ...
    print('Accuracy of model on test images %d %%' % (100*correct/total))

check_model_accuracy(model,testloader)
...
save_model_config('checkpoint.pth')
...
def load_model(path):
    ...
    model,_,_,_=network_model_setup()
